...
export class CreateFactory1691464596829 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`hospital_users\` (\`user_id\` bigint NOT NULL AUTO_INCREMENT, \`username\` varchar(50) NOT NULL COMMENT 'Username', \`password\` varchar(25) NOT NULL COMMENT 'User password', \`date_created\` timestamp(6) NOT NULL COMMENT 'Date created' DEFAULT CURRENT_TIMESTAMP(6), \`date_updated\` timestamp(6) NOT NULL COMMENT 'Date updated' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`role_id\` bigint NULL, PRIMARY KEY (\`user_id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`hospital_meddiagnostics\` (\`correlative_id\` bigint NOT NULL AUTO_INCREMENT, \`symptom_detail\` varchar(210) NOT NULL COMMENT 'detalle de símtomas', \`pre_existing\` varchar(210) NOT NULL COMMENT 'enfermedad preexistente', \`Detected_disease\` varchar(210) NOT NULL COMMENT 'enfermedad detectada', \`date_created\` timestamp(6) NOT NULL COMMENT 'Date created' DEFAULT CURRENT_TIMESTAMP(6), \`date_updated\` timestamp(6) NOT NULL COMMENT 'Date updated' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`member_number\` bigint NULL, \`prescription_id\` bigint NULL, UNIQUE INDEX \`REL_3bd0ff612b310b5ce6797cd7e8\` (\`prescription_id\`), PRIMARY KEY (\`correlative_id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`hospital_medconsultations\` (\`consultation_id\` bigint NOT NULL AUTO_INCREMENT, \`correlative\` bigint NOT NULL COMMENT 'Correlativo de consultas', \`reason\` varchar(120) NOT NULL COMMENT 'razón o motivo de consulta', \`date_consultation\` timestamp(6) NOT NULL COMMENT 'fecha de consulta' DEFAULT CURRENT_TIMESTAMP(6), \`date_created\` timestamp(6) NOT NULL COMMENT 'Date created' DEFAULT CURRENT_TIMESTAMP(6), \`date_updated\` timestamp(6) NOT NULL COMMENT 'Date updated' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`member_number\` bigint NULL, \`correlative_id\` bigint NULL, UNIQUE INDEX \`REL_2302c6084ede85b0e263ec6198\` (\`correlative_id\`), PRIMARY KEY (\`consultation_id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`hospital_meddiagnostics\` ADD CONSTRAINT \`FK_348b33bfca3cd8a082f3ade8af8\` FOREIGN KEY (\`member_number\`) REFERENCES \`hospital_doctors\`(\`personal_id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`hospital_medconsultations\` ADD CONSTRAINT \`FK_2c043a168b8620b10976ccce65e\` FOREIGN KEY (\`member_number\`) REFERENCES \`hospital_doctors\`(\`personal_id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`hospital_medconsultations\` DROP FOREIGN KEY \`FK_2c043a168b8620b10976ccce65e\``);
        await queryRunner.query(`ALTER TABLE \`hospital_meddiagnostics\` DROP FOREIGN KEY \`FK_3bd0ff612b310b5ce6797cd7e8e\``);
        await queryRunner.query(`ALTER TABLE \`hospital_meddiagnostics\` DROP FOREIGN KEY \`FK_348b33bfca3cd8a082f3ade8af8\``);
